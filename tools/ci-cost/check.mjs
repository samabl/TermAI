...
// Validates schema completeness, recomputes every per-pipeline cost and the monthly total, and
// applies A-PM-12's thresholds: warn at 80% of the cap, require downsampling at 100%.
// No dependencies. Exits non-zero only when the cap is reached or exceeded, or the manifest is
// internally inconsistent - a non-gating reading must not turn the build red.
...
console.log('ci-cost: pipelines=' + (c.pipelines || []).length + ' monthly=$' + sum +
  ' cloud_cap=$' + caps.cloud + ' fraction=' + frac.toFixed(3) + ' state=' + state);
if (state === 'WARN') console.log('ci-cost: WARNING - past ' + (warnAt * 100) + '% of the cloud cap; ADR-0014 requires an alert (A-PM-12)');

if (fail.length) {
  for (const m of fail) console.error('ci-cost FAIL: ' + m);
  process.exit(1);
}
