...
| **W5-C** | §5 **H12 输入字节等价**测量（E-P0-3） | **完成，且判出 FAIL**（`1e2977a`） | 75 例语料**每条期望都带依据**（kernel/05 条款或 xterm ctlseqs 规则），11 条无可引依据者**显式列为 `omitted`**；driver 链**真实** `termai_core` rlib 且**不含编码逻辑**；`bench:check --report` → **[B8] FAIL、`H12 = 93.3333 pct`、`summary: 8 PASS / 1 FAIL`**。**5 处真实分歧各有条款**（MOK(2) 的 Ctrl+Shift+A / Alt+x / Esc、Kitty 的 Enter / Left）。`bench:selftest` **78/78 → 83/83**；注入（改坏期望）→ 92%/FAIL，对照（编码器满足的 70 例）→ 100%/PASS | **该行当前 verdict 是 FAIL，不是达标**；未覆盖：原生 IME 宿主、死键组合、FocusRouter/IN-04、OSC 52 读、DECSET 鼠标状态机、11 条无可引依据的期望；`gatingNumbersProduced` **仍为 0**（H12 是 `machine:'none'`，ADR-0029 D-5 不计入机器绑定计数）——**是「§5 门禁行被判定」，不是「机器绑定门禁数字」** |
