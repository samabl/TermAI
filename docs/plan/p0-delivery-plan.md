...
19. **要做成门禁的检查，先问它的答案里有没有判断成分**（第 140、222、234 轮）：**有判断成分的检查只能是候选清单；没有判断成分的才能是门禁；两者都做不到的，就不做。** **三次实例**：① **第 140 轮**按步骤名推断「什么算门禁」→ **13 条假警报**（后改为显式清单）；② **第 222 轮**删除线检查 → **约 2/5 是判断问题** → **做成候选清单（exit 0）**；③ **第 234 轮**范围声明检查 → **3/3 是「引用旧事实」而非「旧断言」** → **回滚，不做**。**判据是**：**一个机械模式若分不清「断言」与「引用」，它就会在**记录过自己错误的仓库**里频繁误报**——**而总在喊的检查会教人忽略它**。**
