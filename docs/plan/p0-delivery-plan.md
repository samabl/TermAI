...
22. **测试依赖宿主资源时，必须把「宿主提供了什么」与「被测规则保证了什么」分开**（第 272 轮第三次同类事故换来）：本会话三次 CI 红都是同一个形状——**本机能过、别的宿主必挂**：① `tests/shaping.rs` 要求系统存在「等宽且覆盖 CJK」的字体（`ubuntu-latest`/`macos-latest` 没有）；② RP-05 摆放测试断言「字形墨迹框必须落在 cell box 内 1px」——而墨迹框是轮廓**向外取整**到整像素、cell box 是分数宽度，**任何**摆放都无法让一个约 1px 宽于 cell 的字形满足它（270 个系统字体里 153 个会失败，等宽类 6/23）；③ 更早的 `TREE_PROCESSES`（Unix 只列组长）。**做法**：**被测规则保证的**（如「摆放偏差 ≤0.5px」「样本数 == 绘制字形数」「零样本必须拒绝」「注入必须越界」）**无条件断言**；**宿主资源提供的**（字体是否等宽/覆盖某字形/墨迹是否宽于 cell、是否有 GPU 适配器、locale、时钟分辨率）**按分支断言 + 打印所取分支与数值**，且在**分支不可用时明确说明该断言在此处为空而非悄悄跳过**。**判据**：一条测试如果在「宿主换了」之后从绿变红，那它测的是宿主，不是代码。
