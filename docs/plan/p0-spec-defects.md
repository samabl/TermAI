...
## SD-20｜esctest-over-adapter 的通过数不可复现（记录 201，当前同一命令为 110）

- **证据**：同一 esctest2 检出（@2798f121）、同一适配器、同一命令，在当前工作区**连续两次**运行结果**逐字段相同**：`110 passed / 43 known bugs / 414 TESTS FAILED / feeds=34024 / substitutions=688`。而 W1-B 收口时记录在 `tools/conformance/upstream/README.md` 的是 **201 passed / 41 known bugs / 325 failed**。**91 个通过用例的差额无法归因。**
- **影响**：esctest 的通过数**当前不能作为任何 G1 门禁证据**。按 AR-27 的口径，测量本身必须先自证可复现**且跨状态差异可解释**；本项跨状态差 91 个用例而无解释，属 **INVALID**。
- **本 P0 处置**：两个数字**都如实登记**，任何报告引用 201 时必须同时注明「不可复现」（本文件与计划的 G1 小节均已如此标注）；在差额定位前，esctest 结果只作**方向性参考**。
- **需要的动作**：二分定位差额来源。候选（按可能性排序）：① W1-B 收口前对适配器/CHECKSUM（DECRQCRA）实现的后期改动——更严格的真实网格校验会把「靠宽松校验通过」的用例判红；② 其后落地的 VT 修复 `e141127`（DECALN/HPA/HPR/REP 与 intermediates 语义）；③ esctest 检出的工作区状态。定位方法：分别在 `7fc89dd` 与 `e141127` 上各跑一次同一命令对比。**定位前不得把 esctest 接入门禁。**
