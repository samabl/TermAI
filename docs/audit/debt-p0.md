...
| **E-P0-3** | 未判定：B-10 方法学已落地并自证（`bench:check` 7 PASS / ~~`bench:selftest` 54/54~~ → **第 255 轮为 63/63**），**§5 机器无关行的读取路径已落地**，但**实测仍缺**（本机只产得出 H18 与表外对照 C1）、无 RM-A/B/C → 每次打印 `gating numbers produced: 0`（**该 0 第 255 轮起是计算值，不是常量**） | ✅ **一致**（第 97 轮再次确认该数为 NON_GATING） |
